...
namespace MyTaskManagement.Persistence.Repositories
{
    ...
    public class UserRepository : Repository<ApplicationUser>, IUserRepositry
    {
        ...
        public IEnumerable<ApplicationUser> GetAllUsersWithProjectsAndTasksAndRolesAndFinanicalWithFilesWithPayments()
        {
            return CuurentContext.Users.Include(user => user.Tasks)
                .Include(user => user.Projects).Include(user => user.Roles).Include(user => user.Payments).Include(user => user.FinancialstatusList).Include(user => user.MyFiles).ToList();
        }

        public  ApplicationUser GetUserWithProjectsAndTasksAndRolesAndFilesAndFinanicalWithFilesWithPayments(string id)
        {
            return CuurentContext.Users.Where(user => user.Id == id).Include(user => user.Tasks)
                .Include(user => user.Projects).Include(user => user.Roles).Include(user => user. Payments).Include(user => user.MyFiles).Include(user => user.MyFiles).Include(user => user.FinancialstatusList).SingleOrDefault();
        }
